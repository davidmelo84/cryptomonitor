  ...
  const createBot = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`❌ Erro ao criar bot: ${errorData.message || 'Erro desconhecido'}`);
      }
    } catch (error) {
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{
                    fontSize: '18px',
                    fontWeight: 'bold',
                    color: (bot.totalProfitLoss || 0) >= 0 ? '#10b981' : '#ef4444'
                  }}>
                    ${(bot.totalProfitLoss || 0).toFixed(2)}
                  </div>
                </div>
              </div>
...
function BotDetailsModal({ bot, trades, isRefreshing, onClose, onRefresh, onStart, onStop, onDelete }) {
  const styles = {
    modal: {
      position: 'fixed',
      top: 0,
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              fontSize: '16px',
              fontWeight: 'bold',
              color: bot.status === 'RUNNING' ? '#10b981' : '#666'
            }}>
              {bot.status === 'RUNNING' ? '● Ativo' : '○ Parado'}
            </div>
          </div>
          ...
          <div style={{
            background: (bot.totalProfitLoss || 0) >= 0 ? '#d1fae5' : '#fee2e2',
            padding: '15px',
            borderRadius: '10px'
          }}>
            <div style={{fontSize: '12px', color: '#666', marginBottom: '5px'}}>
        ...
        {bot.totalTrades > 0 && (
          <div style={{
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{fontSize: '24px', fontWeight: 'bold', color: '#10b981'}}>
                  {(((bot.winningTrades || 0) / bot.totalTrades) * 100).toFixed(1)}%
                </div>
              </div>
              <div style={{textAlign: 'right'}}>
                <div style={{fontSize: '12px', color: '#10b981'}}>
                  ✓ Ganhos: {bot.winningTrades || 0}
                </div>
                <div style={{fontSize: '12px', color: '#ef4444'}}>
                  ✗ Perdas: {bot.losingTrades || 0}
                </div>
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{
                width: `${((bot.winningTrades || 0) / bot.totalTrades) * 100}%`,
                height: '100%',
