...
public class CoinGeckoApiService {
     ...
     *
     * Busca TODAS as moedas em 1 única request
     */
    ...
    public List<CryptoCurrency> getTopPrices(int limit) {
        try {
            ...
            String url = String.format(
                    "%s/coins/markets?vs_currency=usd" +
                            "&order=market_cap_desc" +
                            "&per_page=%d" +
                            "&page=1" +
                            "&price_change_percentage=1h,24h,7d",
    ...
    public List<Map<String, Object>> getHistory(String coinId, int days) {
        try {
            ...
            if (response != null && response.containsKey("prices")) {
                ...
                // ✅ Converter com segurança cada ponto de histórico
                List<Map<String, Object>> history = prices.stream()
                        .filter(p -> p.size() >= 2 && p.get(0) instanceof Number && p.get(1) instanceof Number)
                        .map(p -> {
                            Map<String, Object> point = new HashMap<>();
                            point.put("timestamp", ((Number) p.get(0)).longValue());
                            point.put("price", ((Number) p.get(1)).doubleValue());
                            return point;
                        })
                        .collect(Collectors.toList());

                log.info("✅ Histórico obtido com sucesso: {} pontos", history.size());
                return history;
            }

            log.warn("⚠️ Nenhum dado de histórico encontrado para {}", coinId);
    ...
    /**
     * ✅ CONVERTER formato CoinGecko → CryptoCurrency
     */
    ...
    /**
     * ✅ RATE LIMIT CONTROL - Esperar 500ms entre requests
     */
    ...
    public boolean isAvailable() {
        try {
            ...
            boolean available = response != null &&
                    "pong".equals(response.get("gecko_says"));
